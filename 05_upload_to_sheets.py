...
#         # 5. Create dated backup copy - FIXED VERSION
#         backup_name = f"{datetime.now().strftime('%d-%m-%Y')}_{CONFIG['spreadsheet_name']}"
        
#         # Create copy without copying permissions to avoid owner transfer issues
#         new_file = gc.copy(
#             file_id=sh.id,
#             title=backup_name,
#             copy_permissions=False,  # Changed from True to False
#             folder_id=CONFIG['daily_reports_folder_id']
#         )
        
#         # Add viewer permission to your personal email if needed
#         # new_file.share('your-email@example.com', perm_type='user', role='writer')
