...
def compare_events(old_file: str, new_file: str) -> Dict[str, Any]:
    ...
    return {
        'all_current_events': list(new_events.values()),  # All events from today
        'existing_events': [new_events[url] for url in new_urls & old_urls],  # Events present both days
        'removed': [old_events[url] for url in old_urls - new_urls],
        'stats': {
            'total_current': len(new_events),
            'existing': len(new_urls & old_urls),
...
def generate_report_content(results: Dict[str, Any]) -> str:
    ...
    report_lines.append(f"New file: {stats['total_current']} events")
    report_lines.append(f"Comparison time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    # Changed "All Current Events" to "Newly Added Events" (your request)
    report_lines.append(f"Newly Added Events ({stats['total_current']}):")
    for event in results['all_current_events']:
        is_existing = any(e['url'] == event['url'] for e in results['existing_events'])
        status_marker = " (Existing)" if is_existing else ""
        
        report_lines.append(f"- {event.get('name', 'Untitled Event')}{status_marker}")
